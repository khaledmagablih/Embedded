...
void Delay_us_New(unsigned int us) {
    unsigned int i;
    while(us--) {
        // each iteration here is ~0.5 µs, so 2 ˜1 µs
        for (i = 0; i < 2; i++) {
            ;   // nothing
        }
    }
...
void PWM_Software(unsigned char duty_percent) {
    ...
    Delay_us_New(duty_percent * 10);                 // ON time
    PORTD = PORTD & ~MOTOR_ENABLE_MASK;                          // clear RD0
    Delay_us_New((100 - duty_percent) * 10);         // OFF time
}
...
void interrupt() {
    if (INTCON & 0b00100000) {
        INTCON = INTCON & 0b11011111;    // clear overflow flag
...
void main() {
    ...
    TMR0       = 6;           // preload
    INTCON = INTCON | 0b00010000;          // enable TMR0 interrupt
    INTCON = INTCON | 0b10000000;         // global interrupts
